...
import { ShieldCheck, Plus, Loader2, CheckCircle, Download, Filter, Pencil, Trash2 } from 'lucide-react';
...
const SecurityBills = () => {
  ...
  const [editing, setEditing] = useState<SecurityBill | null>(null);
  ...
  const handleAdd = async () => {
    if (!form.apartment_id || !form.amount) { toast.error(t('common.fillAll')); return; }
    setSaving(true);
  ...
  const openEdit = (bill: SecurityBill) => {
    setEditing(bill);
  ...
  const handleUpdate = async () => {
    if (!editing) return;
    setSaving(true);
    const { error } = await supabase.from('security_bills').update({
  ...
  const handleDelete = async () => {
    if (!deleting) return;
    const { error } = await supabase.from('security_bills').delete().eq('id', deleting.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {filteredBills.length === 0 && <p className="col-span-full text-center text-muted-foreground py-8">{t('bill.noSecurity')}</p>}
      </div>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{t('bill.edit')} - {t('nav.security')}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium">{t('bill.month')}</label>
