...
const Revenue = () => {
  ...
  const barData = [
    { name: t('rev.rent'), Collected: rentRevenue.paid, Pending: rentRevenue.pending },
  ...
  const pieData = [
    { name: t('rev.rent'), value: rentRevenue.paid },
    { name: t('nav.electricity'), value: elecRevenue.paid },
    { name: t('nav.water'), value: waterRevenue.paid },
  ].filter(d => d.value > 0);

  const cards = [
    { icon: Building2, label: t('rev.rent'), paid: rentRevenue.paid, pending: rentRevenue.pending, color: 'text-primary' },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Button onClick={handleDownloadReport} className="gold-gradient text-card">
          <Download className="w-4 h-4 mr-1" /> {t('rev.downloadReport')}
        </Button>
      </div>
      ...
      <Card className="gold-gradient">
        <CardContent className="pt-6 pb-6 text-card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sm opacity-80">{t('rev.totalRevenue')}</p>
              <p className="text-3xl font-bold">{totalPaid.toLocaleString()} {t('common.birr')}</p>
            </div>
          </div>
          {totalPending > 0 && (
            <p className="text-sm opacity-80 mt-1">
              {t('rev.pending')}: {totalPending.toLocaleString()} {t('common.birr')}
            </p>
          )}
        </CardContent>
      </Card>
            ...
            <CardContent>
              ...
              {c.pending > 0 && (
                <p className="text-sm text-muted-foreground mt-1">
                  {t('rev.pending')}: {c.pending.toLocaleString()} {t('common.birr')}
                </p>
              )}
            </CardContent>
          </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">{t('rev.breakdown')}</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={barData}>
                ...
                <Tooltip formatter={(value: number) => `${value.toLocaleString()} ${t('common.birr')}`} />
                <Legend />
                <Bar dataKey="Collected" name={t('rev.collected')} fill="hsl(43, 74%, 49%)" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Pending" name={t('rev.pending')} fill="hsl(var(--muted-foreground))" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">{t('rev.distribution')}</CardTitle>
          </CardHeader>
          <CardContent className="flex items-center justify-center">
            {pieData.length > 0 ? (
              ...
              <p className="text-muted-foreground py-12">{t('rev.noData')}</p>
            )}
          </CardContent>
        </Card>
      </div>
